...
package org.tisonkun.nymph.rpc.network.server;

import static org.tisonkun.nymph.rpc.network.util.NettyUtils.getRemoteAddress;
...
import org.tisonkun.nymph.rpc.network.buffer.ManagedBuffer;
import org.tisonkun.nymph.rpc.network.buffer.NioManagedBuffer;
import org.tisonkun.nymph.rpc.network.client.MergedBlockMetaResponseCallback;
import org.tisonkun.nymph.rpc.network.client.RpcResponseCallback;
import org.tisonkun.nymph.rpc.network.client.StreamCallbackWithID;
import org.tisonkun.nymph.rpc.network.client.StreamInterceptor;
import org.tisonkun.nymph.rpc.network.client.TransportClient;
import org.tisonkun.nymph.rpc.network.io.TransportFrameDecoder;
import org.tisonkun.nymph.rpc.network.protocol.ChunkFetchRequest;
import org.tisonkun.nymph.rpc.network.protocol.Encodable;
import org.tisonkun.nymph.rpc.network.protocol.MergedBlockMetaRequest;
import org.tisonkun.nymph.rpc.network.protocol.MergedBlockMetaSuccess;
import org.tisonkun.nymph.rpc.network.protocol.OneWayMessage;
import org.tisonkun.nymph.rpc.network.protocol.RequestMessage;
import org.tisonkun.nymph.rpc.network.protocol.RpcFailure;
import org.tisonkun.nymph.rpc.network.protocol.RpcRequest;
import org.tisonkun.nymph.rpc.network.protocol.RpcResponse;
import org.tisonkun.nymph.rpc.network.protocol.StreamFailure;
import org.tisonkun.nymph.rpc.network.protocol.StreamRequest;
import org.tisonkun.nymph.rpc.network.protocol.StreamResponse;
import org.tisonkun.nymph.rpc.network.protocol.UploadStream;
...
public class TransportRequestHandler extends MessageHandler<RequestMessage> {
    ...
    private void processStreamRequest(final StreamRequest req) {
        if (logger.isTraceEnabled()) {
            logger.trace("Received req from {} to fetch stream {}", getRemoteAddress(channel), req.streamId);
        }
        ...
        if (maxChunksBeingTransferred < Long.MAX_VALUE) {
            ...
            if (chunksBeingTransferred >= maxChunksBeingTransferred) {
                logger.warn(
                        "The number of chunks being transferred {} is above {}, close the connection.",
    ...
    private void processStreamUpload(final UploadStream req) {
        ...
        try {
            ...
            StreamCallbackWithID wrappedCallback = new StreamCallbackWithID() {
                ...
                public void onComplete(String streamId) throws IOException {
                    try {
                        ...
                        // encoded in the RPCFailure. This type of exception gets thrown more frequently
            ...
            if (req.bodyByteCount > 0) {
                StreamInterceptor<RequestMessage> interceptor =
                        new StreamInterceptor<>(this, wrappedCallback.getID(), req.bodyByteCount, wrappedCallback);
            ...
            if (e instanceof BlockPushNonFatalFailure) {
                ...
                respond(new RpcResponse(
                        req.requestId, new NioManagedBuffer(((BlockPushNonFatalFailure) e).getResponse())));
            } else {
                logger.error("Error while invoking RpcHandler#receive() on RPC id " + req.requestId, e);
                respond(new RpcFailure(req.requestId, Throwables.getStackTraceAsString(e)));
            }
    ...
    private void processMergedBlockMetaRequest(final MergedBlockMetaRequest req) {
        try {
            rpcHandler
                    ...
                    .receiveMergeBlockMetaReq(reverseClient, req, new MergedBlockMetaResponseCallback() {
            ...
            logger.error(
                    "Error while invoking receiveMergeBlockMetaReq() for appId {} shuffleId {} " + "reduceId {}",
                    req.appId,
                    req.shuffleId,
                    req.appId,
                    e);
    ...
    private ChannelFuture respond(Encodable result) {
        ...
        return channel.writeAndFlush(result).addListener(future -> {
            if (future.isSuccess()) {
                ...
                logger.error(
                        String.format("Error sending result %s to %s; closing connection", result, remoteAddress),
                        future.cause());
                channel.close();
            }
        });
