...
import org.tisonkun.nymph.rpc.network.TransportContext;
import org.tisonkun.nymph.rpc.network.config.TransportConfig;
import org.tisonkun.nymph.rpc.network.server.TransportChannelHandler;
import org.tisonkun.nymph.rpc.network.util.IOMode;
import org.tisonkun.nymph.rpc.network.util.JavaUtils;
import org.tisonkun.nymph.rpc.network.util.NettyMemoryMetrics;
import org.tisonkun.nymph.rpc.network.util.NettyUtils;
...
public class TransportClientFactory implements Closeable {
    ...
    public TransportClient createClient(String remoteHost, int remotePort, boolean fastFail)
        ...
        ClientPool clientPool =
                connectionPool.computeIfAbsent(unresolvedAddress, key -> new ClientPool(numConnectionsPerPeer));
        int clientIndex = rand.nextInt(numConnectionsPerPeer);
        TransportClient cachedClient = clientPool.clients[clientIndex];

        if (cachedClient != null && cachedClient.isActive()) {
            // Make sure that the channel will not time out by updating the last use time of the
            ...
            if (cachedClient.isActive()) {
                log.trace("Returning cached connection to {}: {}", cachedClient.getSocketAddress(), cachedClient);
                return cachedClient;
            }
        }
        ...
        if (hostResolveTimeMs > 2000) {
            log.warn("DNS resolution {} for {} took {} ms", resolvMsg, resolvedAddress, hostResolveTimeMs);
        } else {
            log.trace("DNS resolution {} for {} took {} ms", resolvMsg, resolvedAddress, hostResolveTimeMs);
        }
        ...
        synchronized (clientPool.locks[clientIndex]) {
            ...
            if (cachedClient != null) {
                if (cachedClient.isActive()) {
                    log.trace("Returning cached connection to {}: {}", resolvedAddress, cachedClient);
                    return cachedClient;
                } else {
            ...
            if (fastFail && System.currentTimeMillis() - clientPool.lastConnectionFailed < fastFailTimeWindow) {
                throw new IOException(String.format(
                        "Connecting to %s failed in the last %s ms, fail this connection directly",
    ...
    TransportClient createClient(InetSocketAddress address) throws IOException, InterruptedException {
        log.debug("Creating new connection to {}", address);
        ...
        log.debug("Connection to {} successful, running bootstraps...", address);
        try {
            ...
            log.error("Exception while bootstrapping client after " + bootstrapTimeMs + " ms", e);
            client.close();
