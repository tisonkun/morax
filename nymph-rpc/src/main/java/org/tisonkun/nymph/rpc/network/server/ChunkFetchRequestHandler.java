...
import org.tisonkun.nymph.rpc.network.protocol.ChunkFetchFailure;
import org.tisonkun.nymph.rpc.network.protocol.ChunkFetchRequest;
import org.tisonkun.nymph.rpc.network.protocol.ChunkFetchSuccess;
 ...
 * send ChunkFetchRequest to the server at the same time, it could potentially occupy all
...
public class ChunkFetchRequestHandler extends SimpleChannelInboundHandler<ChunkFetchRequest> {
    ...
    public void processFetchRequest(final Channel channel, final ChunkFetchRequest msg) throws Exception {
        ...
        try {
            ...
            logger.error(
                    String.format(
                            "Error opening block %s for request from %s", msg.streamChunkId, getRemoteAddress(channel)),
                    e);
        ...
        respond(channel, new ChunkFetchSuccess(msg.streamChunkId, buf))
                .addListener((ChannelFutureListener) future -> streamManager.chunkSent(msg.streamChunkId.streamId));
    }
