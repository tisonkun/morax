...
public record RequestMessage(ControllerRequestType type, GeneratedMessageV3 message) implements Message {
    ...
    public ByteString getContent() {
        final ByteBuf requestType = Unpooled.buffer().writeInt(type.getNumber());
        final ByteString message = BufferUtils.byteStringDoShade(message().toByteString());
        return BufferUtils.byteBufToByteString(requestType).concat(message);
    }
    ...
    public static RequestMessage fromBytes(ByteString bytes) throws InvalidProtocolBufferException {
        ...
        final var content = BufferUtils.byteStringUndoShade(bytes.substring(Integer.BYTES));
        final GeneratedMessageV3 message =
                switch (type) {
                    case RegisterBookie -> RegisterBookieRequest.parseFrom(content);
                    case ListBookies -> ListBookiesRequest.parseFrom(content);
                    case UNRECOGNIZED -> throw new InvalidProtocolBufferException("unrecognized request type");
                    case null -> throw new InvalidProtocolBufferException("type is null");
                };
