...
class AuthEngine implements Closeable {
    ...
    AuthEngine(String appId, String preSharedSecret, TransportConfig conf) {
     ...
     */
    private AuthMessage encryptEphemeralPublicKey(byte[] ephemeralX25519PublicKey, byte[] transcript)
        ...
        byte[] derivedKeyEncryptingKey =
                Hkdf.computeHkdf(MAC_ALGORITHM, preSharedSecret, nonSecretSalt, aadState, AES_GCM_KEY_SIZE_BYTES);
     ...
     */
    private byte[] decryptEphemeralPublicKey(AuthMessage encryptedPublicKey, byte[] transcript)
            throws GeneralSecurityException {
        ...
        byte[] derivedKeyEncryptingKey = Hkdf.computeHkdf(
                MAC_ALGORITHM, preSharedSecret, encryptedPublicKey.salt, aadState, AES_GCM_KEY_SIZE_BYTES);
    ...
    AuthMessage response(AuthMessage encryptedClientPublicKey) throws GeneralSecurityException {
        ...
        byte[] clientPublicKey = decryptEphemeralPublicKey(encryptedClientPublicKey, EMPTY_TRANSCRIPT);
        ...
        AuthMessage ephemeralServerPublicKey = encryptEphemeralPublicKey(
                X25519.publicFromPrivate(serverEphemeralPrivateKey), getTranscript(encryptedClientPublicKey));
        // Compute a shared secret given the client public key and the server private key
        byte[] sharedSecret = X25519.computeSharedSecret(serverEphemeralPrivateKey, clientPublicKey);
        byte[] challengeResponseTranscript = getTranscript(encryptedClientPublicKey, ephemeralServerPublicKey);
     ...
     */
    void deriveSessionCipher(AuthMessage encryptedClientPublicKey, AuthMessage encryptedServerPublicKey)
            throws GeneralSecurityException {
        Preconditions.checkArgument(appId.equals(encryptedClientPublicKey.appId));
        ...
        byte[] serverPublicKey =
                decryptEphemeralPublicKey(encryptedServerPublicKey, getTranscript(encryptedClientPublicKey));
        // Compute a shared secret given the client public key and the server private key
    ...
    private TransportCipher generateTransportCipher(byte[] sharedSecret, boolean isClient, byte[] transcript)
            throws GeneralSecurityException {
    ...
    private byte[] getTranscript(AuthMessage... encryptedPublicKeys) {
        ByteBuf transcript = Unpooled.buffer(Arrays.stream(encryptedPublicKeys)
                .mapToInt(k -> k.encodedLength())
                .sum());
