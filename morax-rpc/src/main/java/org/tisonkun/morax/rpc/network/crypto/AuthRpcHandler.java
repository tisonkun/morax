...
import org.tisonkun.morax.rpc.network.sasl.SaslRpcHandler;
import org.tisonkun.morax.rpc.network.sasl.SecretKeyHolder;
import org.tisonkun.morax.rpc.network.server.AbstractAuthRpcHandler;
...
class AuthRpcHandler extends AbstractAuthRpcHandler {
    ...
    AuthRpcHandler(TransportConfig conf, Channel channel, RpcHandler delegate, SecretKeyHolder secretKeyHolder) {
        super(delegate);
        this.conf = conf;
    ...
    protected boolean doAuthChallenge(TransportClient client, ByteBuffer message, RpcResponseCallback callback) {
        ...
        try {
            ...
            if (conf.saslFallback()) {
                LOG.warn(
                        "Failed to parse new auth challenge, reverting to SASL for client {}.",
