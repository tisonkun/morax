...
public class OneForOneStreamManager extends StreamManager {
    ...
    private static class StreamState {
        ...
        // Indicates whether the buffers are only materialized when next() is called. Some buffers like
        ...
        StreamState(
                String appId, Iterator<ManagedBuffer> buffers, Channel channel, boolean isBufferMaterializedOnNext) {
            this.appId = appId;
    ...
    public ManagedBuffer getChunk(long streamId, int chunkIndex) {
        ...
        if (state == null) {
            throw new IllegalStateException(
                    String.format("Requested chunk not available since streamId %s is closed", streamId));
        } else if (chunkIndex != state.curChunk) {
            throw new IllegalStateException(
                    String.format("Received out-of-order chunk index %s (expected %s)", chunkIndex, state.curChunk));
